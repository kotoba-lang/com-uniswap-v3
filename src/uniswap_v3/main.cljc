(ns uniswap_v3.main
  "Kotodama WASM entrypoint for the Uniswap_v3 clean-room actor (L4) — Clojure port.
  ...
  facts (`uniswap_v3.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "uniswap_v3")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"uniswap_v3.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "uniswap_v3-compat" :tier tier :entities entities} 200])
...
(defn start! [] :uniswap_v3-compat/ready)
